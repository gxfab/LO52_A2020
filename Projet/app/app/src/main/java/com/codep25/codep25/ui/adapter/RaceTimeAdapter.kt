...
class RaceTimeAdapter(
    _times: List<RaceTime>
) : RecyclerView.Adapter<RaceTimeViewHolder>() {
    private val times = _times.sortedBy { it.timeMs }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RaceTimeViewHolder {
        val inflater = LayoutInflater.from(parent.context)
        val binding = RaceTimeItemBinding.inflate(inflater, parent, false)
        return RaceTimeViewHolder(binding)
    }

    override fun onBindViewHolder(holder: RaceTimeViewHolder, position: Int) {
