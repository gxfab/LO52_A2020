...
public class DetailedStatsActivity extends AppCompatActivity {
    ...
    public void initializeData(){
        ...
        allParticipations = db.participateRaceDAO().getParticipationsFromCourse(this.raceId);
    ...
    public void initializeStats(){
        ...
        for(ParticipateRace pr : participationsSprint1){
            if(pr.getRunningOrder() > 1){
                float time = pr.getSprint1() - this.participationsByTeams.get(pr.getTeamNumber()-1).get(pr.getRunningOrder()-2).getObstacle2();
                pr.setSprint1(time);
            }
        }

        for(ParticipateRace pr : participationsObstacle1){
            float time = pr.getObstacle1() - this.participationsByTeams.get(pr.getTeamNumber()-1).get(pr.getRunningOrder()-1).getSprint1();
            pr.setObstacle1(time);
        }

        for(ParticipateRace pr : participationsPitStop){
            float time = pr.getPitStop() - this.participationsByTeams.get(pr.getTeamNumber()-1).get(pr.getRunningOrder()-1).getObstacle1();
            pr.setPitStop(time);
        }

        for(ParticipateRace pr : participationsSprint2){
            float time = pr.getSprint2() - this.participationsByTeams.get(pr.getTeamNumber()-1).get(pr.getRunningOrder()-1).getPitStop();
            pr.setSprint2(time);
        }

        for(ParticipateRace pr : participationsObstacle2){
            float time = pr.getObstacle2() - this.participationsByTeams.get(pr.getTeamNumber()-1).get(pr.getRunningOrder()-1).getSprint2();
